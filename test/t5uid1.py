...
class T5UID1:
    ...
    def check_paused(self):
        ...
        if not self._is_printing:
            return
        # If the printer is not paused, exit this process
        ...
        if self._current_page == "print_status":
        ...
        curtime = self.reactor.monotonic()
        # Step 2: If this is the first iteration since the printer was paused, set the value of self._print_pause_time to "time now"
        if self._print_pause_time < 0 and self.pause_resume.is_paused:
            self._print_pause_time = curtime
        # Step 3: If the printer was paused, but now has been resumed, calculate the total time it was paused
        elif self._print_pause_time >= 0 and not self.pause_resume.is_paused:
            pause_duration = curtime - self._print_pause_time
            # Add the amount of time that the printer was paused to the recorded print start time, 
